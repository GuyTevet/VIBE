...
if __name__ == "__main__":
    ...
    out_path = './visuals/interp_1d_exp_{}_samplerate{}.pkl'.format(model_type, sample_rate)
    ...
    x_sampled = x_new[0::sample_rate].to(device)
    if x_sampled[-1] != seqlen-1:
        x_sampled = torch.cat((x_sampled.to(device), torch.tensor([seqlen-1]).to(device)))
    y_new = chirp_fn(x_new).to(device)

    # define model
    models ={
        'no_sampling': Interpolator(model_type, 'no_sampling', seqlen, max_value, min_offset=-1, max_offset=1).to(device), # offsets ignored
        'non_adaptive': Interpolator(model_type, 'non_adaptive', len(x_sampled), max_value, min_offset=-1, max_offset=1).to(device), # offsets ignored
        'adaptive': Interpolator(model_type, 'adaptive', len(x_sampled), max_value, min_offset=-(sample_rate / 2), max_offset=(sample_rate / 2)).to(device),
        'super_adaptive': Interpolator(model_type, 'super_adaptive', len(x_sampled), max_value, min_offset=10e-3, max_offset=2*sample_rate).to(device),
    }
    ...
    for step_i in tqdm(range(10000)):
