...
class VIBE(nn.Module):
    ...
    def forward(self, input):  # , J_regressor=None):
        ...
        regressor_output = self.regressor(feature) #  , J_regressor=J_regressor, orig_bs=batch_size)
        for s in regressor_output:
            s['pose'] = s['pose'].reshape(batch_size, seqlen, -1, 6)
            s['shape'] = s['shape'].reshape(batch_size, seqlen, 10)
            s['cam'] = s['cam'].reshape(batch_size, seqlen, 3)
